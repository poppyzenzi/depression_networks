...
ANES9297 <- read.dta('/Users/poppygrimes/Library/CloudStorage/OneDrive-UniversityofEdinburgh/Edinburgh/projects/networks/depression_networks/network_temperature/anes_mergedfile_1992to1997.dta') #the file must be in the working directory

#Beliefs 1992
int1992 <- as.numeric(ANES9297$V923644)
int1992[int1992>4] <- NA
...
# Compare all models:
# We investigate the impact on the fit of the model of constraining edges between nodes to be equal across time points
#constraining the external fields to be equal across time points and constraining the temperature 
#(the entropy of the system) to be equal across time points. 
#Additionally, we tested whether a dense network (all nodes are connected) or a 
#sparse network (at least some edges are absent) fits the data best
